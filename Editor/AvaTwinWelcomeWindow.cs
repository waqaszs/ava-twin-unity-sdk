...
namespace AvaTwin.Editor
{
    ...
    public class AvaTwinWelcomeWindow : EditorWindow
    {
        ...
        private void TestConnection()
        {
            ...
            // This file lives under /Editor/ so Unity strips it from shipped
            // builds — mode is unconditionally "editor". We still send bundle_id
            // and platform for parity with the runtime token-mint callers, so
            // server-side analytics see a consistent shape from every code path.
        ...
        private void PollTestConnection()
        {
            ...
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                // Parse editor_quota from the response body so we can show
                // "X/Y used today" in the Test Connection result. JsonUtility
                // returns a default-initialized struct when the JSON has
                // `"editor_quota": null`, so we detect a real payload by
                // checking for a non-empty plan name.
                string respText = request.downloadHandler != null
                    ? request.downloadHandler.text
                    : null;
                if (!string.IsNullOrEmpty(respText))
                {
                    TokenMintResponse parsed = null;
                    try { parsed = JsonUtility.FromJson<TokenMintResponse>(respText); }
                    catch { /* ignore — quota info is non-critical */ }

                    if (parsed != null
                        && parsed.editor_quota != null
                        && !string.IsNullOrEmpty(parsed.editor_quota.plan))
                    {
                        var eq = parsed.editor_quota;
                        if (eq.limit == -1)
                        {
                            testResultMessage +=
                                $"\n\nEditor mode quota: unlimited ({eq.plan} plan).";
                        }
                        else
                        {
                            testResultMessage +=
                                $"\n\nEditor mode quota: {eq.used}/{eq.limit} used today " +
                                $"({eq.remaining} remaining, {eq.plan} plan).";
                        }

                        // Also emit the standard console log via the shared helper so
                        // every token-mint caller (including Test Connection) produces
                        // an identical Unity Console line, regardless of which UI path
                        // surfaced the result.
                        AvaTwin.EditorQuotaLogger.LogIfPresent(parsed.editor_quota);
                    }
                }
            }
        ...
        private struct TokenMintRequest
        {
            ...
            // Hardcoded to "editor" at the call site. This file lives under /Editor/ so
            // Unity strips it from shipped builds — the literal cannot reach production.
            public string mode;
            // v4 contract: bundle_id (Application.identifier) for apps cap enforcement
            // and platform (Application.platform) for analytics. Sent for parity with
            // the runtime token-mint callers in AvatarPipeline / AvaTwinApiClient /
            // PlayerApi so the server sees a consistent shape from every code path.
            public string bundle_id;
            public string platform;
        }

        // Response payload for /api/token-mint. We only deserialize the fields
        // we display (token presence is implied by HTTP 200, and editor_quota
        // is surfaced in the Test Connection result message). See the edge
        // function `customizer-token-mint` for the authoritative shape.
        [System.Serializable]
        private class TokenMintResponse
        {
            public string token;
            public string error;
            public AvaTwin.EditorQuota editor_quota;
        }
