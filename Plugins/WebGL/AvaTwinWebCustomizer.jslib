      ...
      window.addEventListener("message", function (event) {
        ...
        if (typeof event.data === "string") {
          ...
          if (event.data.type === "ava-twin:avatar-saved" && event.data.glb_url) {
            ...
            // Persist avatar_id and skin_tone in the page URL so a full
            // page reload (or cross-scene addressable load) sees them and
            // skips the customizer. Hosts can read these via their normal
            // URL parameter parsing.
            try {
              var pageUrl = new URL(window.location.href);
              if (event.data.avatar_id) pageUrl.searchParams.set("avatar_id", event.data.avatar_id);
              if (event.data.skin_tone) pageUrl.searchParams.set("skin_tone", event.data.skin_tone);
              window.history.replaceState({}, "", pageUrl.toString());
            } catch (e) {
              // Non-fatal — older browsers / locked-down environments.
            }
