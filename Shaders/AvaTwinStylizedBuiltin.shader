Shader "Ava-Twin/Stylized Builtin"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma multi_compile_fog
            // Additive lights never use baked shadowmasks. Stripping these
            // variants prevents Unity from compiling FORWARD_ADD with
            // SHADOWS_SHADOWMASK / LIGHTMAP_SHADOW_MIXING keywords — combos
            // that reference _ShadowCoord, which isn't declared in this
            // pass (SHADOW_COORDS expands empty when those keywords are on
            // without SHADOWS_SCREEN/SHADOWS_DEPTH). Without this skip,
            // host projects that enable Lightmap "Custom" stripping with
            // ShadowMask kept hit "invalid subscript '_ShadowCoord'" on
            // gles3 builds.
            #pragma skip_variants SHADOWS_SHADOWMASK LIGHTMAP_SHADOW_MIXING
