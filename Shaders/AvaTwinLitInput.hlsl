...
// Added for the minimal v1.1.7 ForwardLit pass — keeps SRP Batcher
// CBUFFER layout consistent across all 3 passes (ForwardLit /
// ShadowCaster / DepthOnly all #include this file).
half _SpecStrength;
half4 _SkyColor;
half4 _GroundColor;
half _HemiStrength;
...
inline void InitializeStandardLitSurfaceData(float2 uv, out SurfaceData outSurfaceData)
{
    ...
    // Skin color applied only where mask is present (R channel > 0.5)
    half3 albedo = albedoAlpha.rgb * _BaseColor.rgb;
    if (_HasSkinMask > 0.5)
    {
        half mask = step(0.5, SAMPLE_TEXTURE2D(_SkinMask, sampler_SkinMask, uv).r);
        albedo = lerp(albedo, albedo * _SkinColor.rgb, mask);
    }
    // No mask = no skin tinting (clothing stays original color)
