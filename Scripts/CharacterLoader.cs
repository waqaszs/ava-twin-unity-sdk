...
#pragma warning restore CS0414
    ...
    public async void EditorLoadRandomAvatar()
    {
        try
        {
            ...
            // ResolveAndLoadAvatar uses api_key direct on avatar-resolve — no extra session count.
            // The mint above is for the editor library/save flow only (one mint per editor click).
    ...
    /// <summary>
    /// Resolve avatar via api_key direct, download GLB, instantiate, apply materials, configure humanoid.
    /// Delegates all steps to AvatarPipeline, but retains CharacterLoader-specific state management
    /// (GlbUrl, events, disk cache, loading status). Uses the api_key direct path on avatar-resolve so
    /// game-runtime LoadAvatar flows do NOT count a customizer session per call.
    /// </summary>
    private async Task ResolveAndLoadAvatar(string avatarId)
    {
        ...
        // Step 1: Resolve avatar via api_key direct (no token-mint, no session count).
        // avatar-resolve accepts x-api-key + x-app-id headers for game-runtime load paths.
        LoadingStatusChanged?.Invoke("Resolving avatar...");
        AvatarPipeline.ResolveResult resolveResult;
        try
        {
            resolveResult = await AvatarPipeline.ResolveAvatarByApiKeyAsync(avatarId, appId, apiKey, baseUrl);
        }
        catch (Exception ex)
        {
        ...
        // Step 2: Set GlbUrl (Atlanta reads this for Photon) and load the GLB
    ...
    private string BuildCustomizerUrl()
    {
        ...
        // Add platform metadata — required by server's strict platform check.
        var bundleId = Application.identifier;
        if (!string.IsNullOrWhiteSpace(bundleId))
        {
            sb.Append(separator);
            sb.Append("bundle_id=");
            sb.Append(Uri.EscapeDataString(bundleId));
            separator = "&";
        }

        var platformStr = Application.platform.ToString();
        if (!string.IsNullOrWhiteSpace(platformStr))
        {
            sb.Append(separator);
            sb.Append("platform=");
            sb.Append(Uri.EscapeDataString(platformStr));
        }
