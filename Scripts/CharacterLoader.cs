...
#pragma warning restore CS0414
    [Tooltip("When not on WebGL, open Unity customizer instead of iframe flow.")]
    [SerializeField] private bool useMobileCustomizerOutsideWebGl = true;
    [Tooltip("Optional prefab override for mobile customizer controller.")]
    [SerializeField] private AvaTwinCustomizerController mobileCustomizerControllerPrefab;
    [Tooltip("If prefab override is empty, loads controller prefab from Resources using this path.")]
    [SerializeField] private string mobileCustomizerControllerResourcePath = "AvaTwinCustomizerController";
    [Tooltip("Optional parent for instantiated mobile customizer UI.")]
    [SerializeField] private Transform mobileCustomizerParent;
    [Tooltip("Optional config override passed into mobile customizer.")]
    [SerializeField] private AvaTwinConfig mobileCustomizerConfigOverride;
    ...
    private AvaTwinCustomizerController _mobileCustomizerInstance;
...
#if UNITY_EDITOR
        // Editor: always use resolve + direct load for testing
        EditorLoadRandomAvatar();
        return;
#elif UNITY_WEBGL
        // WebGL builds: iframe customizer
...
#else
        // Mobile/standalone: native customizer UI
        if (TryInitializeMobileCustomizer())
            return;

        if (!string.IsNullOrWhiteSpace(glbUrl))
        {
#if UNITY_EDITOR || DEVELOPMENT_BUILD
            Debug.Log("[Ava-Twin] Standalone mode — loading existing avatar.");
#endif
            LoadCharacter();
        }
        else
        {
            Debug.LogWarning("[Ava-Twin] No GLB URL set. Configure a URL in the Inspector or use the customizer flow.");
            CharacterLoadFailed?.Invoke("No GLB URL available. Set 'Editor Test Glb Url' in Inspector.");
            _isLoading = false;
        }
#endif
    }
    ...
    private bool TryInitializeMobileCustomizer()
    {
        if (!useMobileCustomizerOutsideWebGl)
            return false;

        if (_mobileCustomizerInstance == null)
        {
            var prefab = mobileCustomizerControllerPrefab;
            if (prefab == null && !string.IsNullOrWhiteSpace(mobileCustomizerControllerResourcePath))
                prefab = Resources.Load<AvaTwinCustomizerController>(mobileCustomizerControllerResourcePath);

            if (prefab == null)
            {
                Debug.LogWarning("[Ava-Twin] Mobile customizer controller prefab not found. Assign a prefab or place one in Resources.");
                return false;
            }
        ...
        _mobileCustomizerInstance.Configure(mobileCustomizerConfigOverride, this);
