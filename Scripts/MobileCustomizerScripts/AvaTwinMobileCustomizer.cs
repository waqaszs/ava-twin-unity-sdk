...
public class AvaTwinMobileCustomizer : MonoBehaviour
{
    ...
    private OrientationLayoutController orientationLayoutController;
    ...
    [Header("Preview")]
    AvaTwinPreviewCameraController previewCameraController;
    [Tooltip("Resources path for the preview object prefab (without extension). Contains the preview camera, 4 focus points, and the spawn point the avatar is placed at.")]
    [SerializeField] private string cameraSetupResourcePath = "PreviewObject";
    ...
    private async Task<Texture2D> GetThumb(string variationId)
    {
        ...
        {
            Debug.LogWarning("[Ava-Twin] Failed to load thumbnail." + ex);
            return null;
        }
    }
    ...
    private void EnsureCameraSetupLoaded()
    {
        ...
        // Auto-wire previewSpawnPoint from the PreviewObject prefab's child.
        // The serialized field on the customizer prefab is null because the
        // PreviewObject is instantiated at runtime — wire it here so the
        // avatar spawns at the correct position and camera focus points align.
        if (previewSpawnPoint == null)
        {
            ...
            if (spawnTf != null)
                ...
                Debug.LogWarning("[Ava-Twin] PreviewObject prefab is missing PreviewSpawnPoint child.");
        }
    }
    ...
    private async Task LoadPreviewForCurrentSelectionAsync(int requestId)
    {
        ...
        {
            SetLoadingVisible(false);
            Debug.LogWarning("[Ava-Twin] Failed to load avatar preview." + ex);
        }
    }
    ...
    private async Task LoadAndApplyThumbnailAsync(AvaVariationButton button, string variationId)
    {
        ...
        if (!wasCached)
        {
            ...
            {
                SetLoadingVisible(false);
                // Trigger one more refresh after async icon loading finishes.
                RequestLayoutRefresh();
            }
        }
    }
    ...
    private void RequestLayoutRefresh()
    {
        if (orientationLayoutController == null)
            orientationLayoutController = FindObjectOfType<OrientationLayoutController>();

        if (orientationLayoutController != null)
            orientationLayoutController.RequestLayoutRefresh();
    }
