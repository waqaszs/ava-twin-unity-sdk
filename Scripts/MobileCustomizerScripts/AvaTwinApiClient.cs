...
public class AvaTwinApiClient
{
    ...
    public async Task<string> MintTokenAsync()
    {
        ...
        // (server is in grace mode for missing bundle_id until 2026-06-01).
    ...
    public async Task<Texture2D> GetVariationThumbnailAsync(string variationId)
    {
        ...
        string metaUrl = $"{_config.baseApiUrl}/api/variation-image?id={variationId}";
    ...
    public async Task<SdkAvatarSaveResponse> SdkAvatarSaveAsync(
        string gender, string head, string top, string bottom, string shoes, string skinTone = null)
    {
        EnsureToken();
        var url = $"{_config.baseApiUrl}/api/sdk/avatar-save";

        var body = JsonConvert.SerializeObject(new
        {
            variation_selections = new
            {
                gender = gender,
                head = head,
                top = top,
        ...
        var response = await PostJsonAsync<SdkAvatarSaveResponse>(url, body, SessionToken);
        return response;
    }
    ...
    public async Task<AvatarResolveResponse> ResolveAvatarAsync(string avatarId)
    {
        ...
        // v4 contract: include bundle_id + platform so the server can enforce
        // per-app platform_identifiers allowlists on Category C runtime calls.
