...
namespace AvaTwin
{
    ...
    internal static class PlayerApi
    {
            ...
            string baseUrl, string appId, string apiKey, string deviceId, CancellationToken ct = default)
        {
            ...
            var body = new PlayerGuestInitRequest
            {
                device_id = deviceId,
                external_user_id = AvaTwinPlayer.ExternalUserId,  // null/empty → server uses player_id default (v4 contract)
                bundle_id = bundleId,                              // v4 contract: server-side platform_identifiers enforcement
        ...
        {
            var body = new PlayerAvatarSaveRequest
            {
            ...
            string baseUrl, string appId, string apiKey, string username, CancellationToken ct = default)
        {
            var customizerToken = await MintCustomizerTokenAsync(baseUrl, appId, apiKey, ct);
            var (bundleId, platformStr) = PlatformInfo.Get();
            var body = new PlayerUsernameCheckRequest
            {
                username = username,
                bundle_id = bundleId,    // v4 contract: server-side platform_identifiers enforcement
                platform = platformStr
        ...
        [Serializable]
        private class TokenMintRequest
        {
            public string appId;
            public string apiKey;
            public string mode;        // "editor" in Unity Editor (compile-stripped from prod builds), "embed" otherwise
            public string bundle_id;   // Application.identifier — used by server for apps cap enforcement (v4 contract)
            public string platform;    // Application.platform.ToString() e.g. "WebGLPlayer", "IPhonePlayer", "Android"
        }
        [Serializable]
        private class TokenMintResponse
        {
            ...
            // for embed/playground mints. Read in Editor builds only — the
            // logging block below is wrapped in #if UNITY_EDITOR so the
            // string formatting is stripped from production binaries.
            ...
            string baseUrl, string appId, string apiKey, CancellationToken ct)
        {
            ...
            var bodyJson = JsonUtility.ToJson(new TokenMintRequest
            {
                appId = appId,
                apiKey = apiKey,
#if UNITY_EDITOR
                // Editor flows hit a separate server branch with no session count
                // and relaxed platform checks. The "editor" literal is compile-stripped
                // from shipped builds via the preprocessor — defense-in-depth so devs
                // cannot hijack the platform reporting api by faking editor-mode in production.
                mode = "editor",
#else
                mode = "embed",
#endif
